...
const priceVal = price.text(),
  splitPrice = priceVal.split('.'),
  detailHeight = $('.template-product .detail').outerHeight(),
  bgHeight = $('.template-product .bg').outerHeight(),
  video = $('.bg .plyr');

price.html(splitPrice[0] + '<span>.' + splitPrice[1] + '</span>');
...
function cssVar() {
  ...
  const bgHeight = $('.template-product .bg').outerHeight();
  if (detailHeight != undefined) {

    $('.detail').attr('style', '--height: ' + detailHeight + 'px;');
  }

  if (bgHeight != undefined) {

    $('.template-product .bg').attr('style', '--height: ' + bgHeight);
  }
...
if (video.length) {
  $(window).scroll(function() {
    ...
    const visible = $('.bg').visible(true, true),
      playing = $('.plyr--playing').length,
      headerHeight = $('#shopify-section-header').outerHeight();


    if ((playing == 1) && ($(window).width() > 993)) {
